            ...
            for x, tile in enumerate(row):
                if tile >= 0:
                    ...
                    if tile >= 0 and tile <= 8:
                        ...
                        exit_group.add(exit)
                    elif tile == 21:#create coin
                        coin = Coin(x * TILE_SIZE, y * TILE_SIZE)
                        coin_group.add(coin)
                    elif tile == 22:#create boss
                        boss = Boss(x * TILE_SIZE, y * TILE_SIZE, 2, 2, 20)						
                        boss_group.add(boss)
...
class Boss(pygame.sprite.Sprite):
    def __init__(self, x, y, scale, speed, ammo):
        ...
        animation_types = ['idle', 'range_attack', 'beam_attack', 'walk', 'jump_attack','death', 'take_hit', 'transform', 'not_yet']
        for animation in animation_types:
            #reset temporary list of images
            temp_list = []
            #count number of files in the folder
            num_of_frames = len(os.listdir(f'img/boss/model-2/{animation}'))
            for i in range(num_of_frames):
                img = pygame.image.load(f'img/boss/model-2/{animation}/{i}.png').convert_alpha()
                img = pygame.transform.scale(img, (int(img.get_width() * scale), int(img.get_height() * scale)))
                temp_list.append(img)
            self.animation_list.append(temp_list)
    ...
    def update_animation(self):
        ...
        if self.frame_index >= len(self.animation_list[self.action]):
            if self.action == 5:
                ...
                bullet_group.add(self.shoot_lst.pop(0))
                self.update_action(0)
            elif self.action == 2:
                bullet_group.add(self.shoot_lst.pop(0))
                if self.beam_cd >= 0:
...
class Attack(pygame.sprite.Sprite):
    def __init__(self, x, y, direction, type, scale):
        pygame.sprite.Sprite.__init__(self)
        self.speed = 10
        self.type = type
        self.animation_list = []
        ...
        for animation in attack_type:
            ...
            temp_list = []
            #count number of files in the folder
            num_of_frames = len(os.listdir(f'img/boss/attack/{animation}'))
            for i in range(num_of_frames):
                img = pygame.image.load(f'img/boss/attack/{animation}/{i}.png').convert_alpha()
                img = pygame.transform.scale(img, (int(img.get_width()), int(img.get_height())))
                temp_list.append(img)
            self.animation_list.append(temp_list)
        self.frame_index = 0
        self.image = self.animation_list[self.type][self.frame_index]		
        self.rect = self.image.get_rect()
        self.rect.center = (x, y)
        self.direction = direction
        self.duration = 20
    ...
    def update(self):
        ...
        #beam atk
            ...
            self.image = self.animation_list[self.type][self.frame_index]
            self.frame_index += 1
            if self.frame_index >= len(self.animation_list[self.type]):
                if self.duration >= 0:
                    self.duration -= 1
                    self.frame_index = len(self.animation_list[self.type]) - 2
        elif self.type == 2:
            if self.frame_index >= len(self.animation_list[self.type]):
                self.kill()
...
boss_atk_group = pygame.sprite.Group()
